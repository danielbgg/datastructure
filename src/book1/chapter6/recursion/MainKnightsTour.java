package book1.chapter6.recursion;

public class MainKnightsTour {
	static int firstRow = 4;
	static int firstCol = 0; // starting position of the knight
	static int nRows = 5;
	static int nCols = 5; // number of columns and rows
	static int[][] board = new int[nRows][nCols]; // stores the move number of
												  // the Knight
	static int[][] rowColIncrement = new int[8][2]; // stores valid Knights move
													// choices
	...
	public static boolean thisDecisionChoiceIsValid(int row, int column) {
		if (row >= 0 && row < nRows && column >= 0 && column < nCols && board[row][column] == 0)
			return true;
		else
			return false;
	}

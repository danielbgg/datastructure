package book.dsauj.chapter7.trees;
		...
		else // insert the node
		{
			...
			if (root == null) // tree is empty
			{
				...
				findNode(newListing.getKey(), p, c); // find the new node's
													 // parent
				if (newListing.getKey().compareTo(p.get().node.getKey()) < 0) // insert
																			  // at
																			  // right
					p.get().lc = n;
				else
		...
		if (found == false) // node not found
			...
			else // case 3: deleted node has two children
			{
				...
				if (largest != null) // left child of deleted node has a right
									 // subtree
				{
					while (largest.rc != null) // move down the right edge of
											   // right subtree
					{
						nextLargest = largest;
						largest = largest.rc;
					}
					c.get().node = largest.node; // "copy" largest node's info
												 // into deleted node
					nextLargest.rc = largest.lc; // save left subtree of largest
												 // node
				} else // left child of deleted node does not have a right
					   // subtree
				{
					nextLargest.rc = c.get().rc; // save the right subtree of
												 // the deleted node
					if (p.get().lc == c.get()) // deleted node is a left child
						p.get().lc = nextLargest; // deleted node's parent jumps
												  // around deleted node
					else
						// deleted node is a right child
						p.get().rc = nextLargest; // deleted node's parent jumps
												  // around deleted node
				}
			}// end of case 3
			return true;
